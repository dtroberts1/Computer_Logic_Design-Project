...
static unsigned Mem[MEMSIZE]; // Memory Cache
static unsigned Reg[REGSIZE + 4]; // Register File

// Define Macro expressions
#define MEM(addr) (Mem[addr >> 2]) // When Mem(addr) is called, argument will be inserted into Mem[addr >> 2];

#define PC (Reg[REGSIZE + 0])
#define Status (Reg[REGSIZE + 1]) // not used for now
#define LO (Reg[REGSIZE + 2]) // not used for now
#define HI (Reg[REGSIZE + 3]) // not used for now
...
int main(int argc, char **argv)
{
	...
	for (i = PCINIT; !feof(FP); i += 4)
	{
		...
		else
		{
			...
			//printf("MEM(i) has been set to %x", strtoul(Buf, (char **) NULL, 16));
		}
	}
	Loop();
	fclose(FP);
